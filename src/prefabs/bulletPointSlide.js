...
class BulletPointSlide extends Phaser.Group {
  ...
  constructor(game, titleText, nameText, bulletPoints) {
    super(game, null);
    const title = new CenteredContent(game, titleText);
    const mediumStyle = styles.fonts.medium(game)
    title.setStyle(mediumStyle);
    title.y = 0;
    title.anchor.setTo(0.5, 0);

    const name = new CenteredContent(game, nameText);
    ...
    for (let i = 0; i < bulletPoints.length; ++i) {
        ...
        point.y = point.y + ((1 + i) * 2 * mediumStyle.fontHeight)
        this.add(point);
    }
